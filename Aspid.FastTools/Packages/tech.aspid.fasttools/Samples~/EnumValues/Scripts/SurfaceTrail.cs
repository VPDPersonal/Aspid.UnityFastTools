...
{
    // A flat ribbon on the floor. Each color section keeps its own history, so palette changes do not
    // recolor old points. Time-based trimming moves the tail smoothly instead of removing whole marks.
    public sealed class SurfaceTrail : MonoBehaviour
    {
        ...
        {
            if (!force && _points.Count > 0 && Vector3.Distance(_points[_points.Count - 1], position) < 0.02f) return;
        ...
        private void LateUpdate()
        {
            ...
            for (var i = 0; i < _points.Count; i++)
            {
                ...
                {
                    _triangles.Add(a); _triangles.Add(a + 2); _triangles.Add(a + 1);
                    _triangles.Add(a + 1); _triangles.Add(a + 2); _triangles.Add(a + 3);
                }
                else
                {
